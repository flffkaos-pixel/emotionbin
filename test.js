...
console.log('\n=== 5. Script Load Order ===');
const scriptTags = [...html.matchAll(/<script\s+(?:type="module"\s+)?src="js\/([^"?]+)[^"]*"(?:\s+defer)?><\/script>/g)];
const order = scriptTags.map(m => m[1]);
const expectedOrder = ['supabase-db.js', 'trash-mountain.js', 'ai.js', 'effects.js', 'drum-scene.js', 'app.js'];
